...
void check(da3_status value, const char* operation) {
    if (value != DA3_STATUS_OK) throw std::runtime_error(
        std::string(operation) + " failed: " + da3_last_error());
}
...
SelectedDevice select_device(const ibrh_api& api) {
    ...
    throw std::runtime_error("no D3D12 adapter accepted by DA3 Vulkan");
}
...
Submitted submit(
    ...
    if (result.output.source_frame_id != frame ||
        ...
        throw std::runtime_error("DA3 output descriptor correlation failed");
    return result;
}
...
std::vector<float> resize_align_true(
    ...
    for (std::uint32_t y = 0; y < output_height; ++y) {
        const float sy = output_height > 1u
            ? static_cast<float>(y) * (input_height - 1u) /
                (output_height - 1u) : 0.0f;
        const auto y0 = static_cast<std::uint32_t>(std::floor(sy));
        const auto y1 = std::min(y0 + 1u, input_height - 1u);
        const float fy = sy - y0;
        for (std::uint32_t x = 0; x < output_width; ++x) {
            const float sx = output_width > 1u
                ? static_cast<float>(x) * (input_width - 1u) /
                    (output_width - 1u) : 0.0f;
            const auto x0 = static_cast<std::uint32_t>(std::floor(sx));
            const auto x1 = std::min(x0 + 1u, input_width - 1u);
            const float fx = sx - x0;
            const float top = input[static_cast<std::size_t>(y0) * input_width + x0] *
                    (1.0f - fx) +
                input[static_cast<std::size_t>(y0) * input_width + x1] * fx;
            const float bottom = input[static_cast<std::size_t>(y1) * input_width + x0] *
                    (1.0f - fx) +
                input[static_cast<std::size_t>(y1) * input_width + x1] * fx;
...
int main() try {
    ...
    if ((capabilities.flags & required) != required ||
        ...
        throw std::runtime_error("DA3 GPU capability contract is incomplete");
    ...
    da3_transfer_counters before{sizeof(before), DA3_ABI_VERSION, 0u, 0u};
    check(da3_get_transfer_counters(&before), "transfer counters before");
    ...
    for (std::uint32_t index = 0; index < retained.size(); ++index) {
        ...
        if (status.state != IBRH_JOB_COMPLETE ||
            ...
            throw std::runtime_error("DA3 job completion correlation failed");
    ...
    if (retained[0].output.resource.native_handle ==
        ...
        throw std::runtime_error("retained DA3 leases alias output slots");
    ...
    if (api.submit(model, sizeof(dropped_request), &dropped_request, &dropped_job) !=
        ...
        throw std::runtime_error("DA3 fourth live lease was not rejected");
    ...
    check(da3_inferbridge_image_shape(
        width, height, 140, &reference_width, &reference_height),
        "da3_inferbridge_image_shape(reference)");
    da3_context* cpu = nullptr;
    check(da3_create(model_text.c_str(), &cpu),
          "da3_create(reference)");
    std::vector<float> processed(
        static_cast<std::size_t>(reference_width) * reference_height);
    check(da3_infer_bgra8_f32(
        cpu, reused_pixels.data(), width * 4u, width, height, 140,
        processed.data(), processed.size()), "da3_infer_bgra8_f32(reference)");
    da3_destroy(cpu);
    std::vector<float> reference = resize_align_true(
        processed, static_cast<std::uint32_t>(reference_width),
        static_cast<std::uint32_t>(reference_height), width, height);
    normalize(reference);
    ...
    if (maximum_difference >= 0.01f)
        throw std::runtime_error("DA3 GPU output exceeds the 1% CPU gate");
    ...
    if (cancelled_status.state != IBRH_JOB_CANCELLED)
        throw std::runtime_error("DA3 cancellation state failed");
    ...
    da3_transfer_counters after{sizeof(after), DA3_ABI_VERSION, 0u, 0u};
    check(da3_get_transfer_counters(&after), "transfer counters after");
    if (after.tensor_upload_bytes != before.tensor_upload_bytes ||
        after.tensor_download_bytes != before.tensor_download_bytes)
        throw std::runtime_error("DA3 GPU path performed host tensor staging");

...
#if defined(__APPLE__) || (defined(DA3_WITH_VULKAN) && defined(_WIN32))
    std::shared_ptr<Da3GpuWorker> gpu_worker;
...
#if defined(__APPLE__) || (defined(DA3_WITH_VULKAN) && defined(_WIN32))
    std::shared_ptr<Da3GpuAdmission> gpu_admission;
    std::weak_ptr<Da3GpuWorker> gpu_worker;
...
ibrh_result status_result(da3_status status) {
    switch (status) {
        case DA3_STATUS_OK: return IBRH_OK;
        case DA3_STATUS_INVALID_ARGUMENT:
        case DA3_STATUS_BUFFER_TOO_SMALL:
            return IBRH_ERROR_INVALID_ARGUMENT;
        case DA3_STATUS_VULKAN_UNAVAILABLE:
        case DA3_STATUS_UNSUPPORTED:
...
#if defined(__APPLE__) || (defined(DA3_WITH_VULKAN) && defined(_WIN32))
struct Da3GpuAdmission {
    explicit Da3GpuAdmission(std::shared_ptr<std::atomic<uint32_t>> value)
        : count(std::move(value)) {}
    ~Da3GpuAdmission() { count->fetch_sub(1u); }
    std::shared_ptr<std::atomic<uint32_t>> count;
};

class Da3GpuWorker {
public:
    explicit Da3GpuWorker(da3_context* context)
    ...
    void enqueue(ibrh_job* job) {
        retain_job(job);
        {
            ...
            if (stopping_) {
                release_job(job);
                throw std::runtime_error("DA3 GPU worker is stopping");
            }
    ...
    void retire(std::shared_ptr<da3_native::ExternalJob> job) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
...
private:
    void run() {
        for (;;) {
            ...
            std::deque<std::shared_ptr<da3_native::ExternalJob>> retired;
            {
    ...
    std::deque<std::shared_ptr<da3_native::ExternalJob>> retired_;
    bool exited_ = false;
    ...
    if (json_uint(device, "index", index)) {
        if (index > static_cast<uint32_t>(std::numeric_limits<int32_t>::max())) {
            ...
            return fail(
                ...
                "DA3 requested device index is out of range");
        }
...
ibrh_result IBRH_CALL model_load(
    ...
    *output = nullptr;
    ...
    try {
        model->gpu_worker =
            std::make_shared<Da3GpuWorker>(model->context);
    } catch (...) {
        da3_destroy(model->context);
        delete model;
        return fail(runtime, IBRH_ERROR_INTERNAL,
            "DA3 could not start its GPU worker");
    }
#endif
    *output = model;
    return IBRH_OK;
...
ibrh_result IBRH_CALL submit(ibrh_model* model,size_t request_size,const ibrh_submit_request* request,ibrh_job** output){
 ...
 if(input.domain==IBRH_RESOURCE_DOMAIN_METAL){
  ...
  uint32_t admitted=model->gpu_admissions->load();
  while(admitted<3u&&!model->gpu_admissions->compare_exchange_weak(admitted,admitted+1u)){}
...
struct LinuxCaptureHooks {
    static ibr_linux_capture_capabilities capabilities(ibrh_model* model) {
        ...
        return da3_linux_capture_capabilities(model->context);
    }
    static void infer(ibrh_model* model, const inferbridge::linux_capture::LinuxDmaBufImage& source,
        ...
        if (!input_size(parameters, size, size)) throw std::invalid_argument("invalid da3 input size");
        std::lock_guard<std::mutex> lock(model->submit_mutex);
        da3_infer_linux_capture(model->context, source, size, output);
    }
};
}
